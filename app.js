...
let G = {
  ...
  currentTargetContext: null, layer3Scored: [], layer3SelectionMeta: null
};
let chartInst = null;
...
function renderStoreBar() {
  document.getElementById('storeBar').innerHTML=
    ...
    G.stores.map(s=>`<button class="store-btn ${s===currentStore?'active':''}" onclick="switchStore('${s}',this)">${s==='all'?'全店舗':s}</button>`).join('');
}
