...
- [ ] スマホからSSH経由でClaude Codeを操作できるようにする（keychain問題の解決）

## 2026-04 設計方針変更

### 役割分離
- 朝イチ用MVP（着席順位ツール）と当日設定推測ツールを分離する
- 今後の開発は朝イチ用MVPを優先する

### 朝イチ用MVP方針
- 分析の主役はホールの配分傾向（店×日条件×機種×台番号/末尾）
- 台ごとの長期ボーナス履歴・ベイズ処理・Monte Carlo・動的事前分布は使わない
- morning_compute.pyで集計してmorning_data.jsonに出力する
- 予報タブを朝イチMVP表示に置き換える

### 仮ラベル定義
- 強上候補：G数5000以上・RB設定4相当以上・合算設定4相当以上
- 上候補：G数3500以上・RB設定4相当以上またはの合算設定4相当以上
- 除外寄り：G数2500未満・BBだけ強い台・差枚だけ良い台

### 席配置タブの扱い
- 分析ロジックの入力ではなく視覚化UIとして使う
- 候補台のヒートマップ表示のみ
